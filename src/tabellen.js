...
<div class="srsPlanVerein" srsKlasse="BOLWJA" srsVerein="Zirndorf" srsAlle=1></div>
...
jQuery(document).ready(function($){
	...
	var show_tables = function(div, data, p) {
		...
		if (data.Tabelle || data.Aktueller_Spielplan && data.Aktueller_Spielplan.Tabelle) {
			...
			p.classCol = p.tabellenformat || "c;l;c;c;c;c;c;c;c;c";
			if (+p.minitab) {
	...
	// diese ausgeben als datum, zeit, heim, gast
	...
	var show_plan = function(div, data, p) {
		...
		if (data && data.Spielplan && data.Spielplan.Spielplan) {
			...
			p.header = p.tabellenkopf || (p.spielplanverein ? "Datum;Zeit;Liga;Heim;Gast;Ergebnis" : "Datum;Zeit;Heim;Gast;Ergebnis");
			p.fields = p.tabellenspalten || (p.spielplanverein ? "Spieldatum/^..(..).(..).(..).*/$3.$2.$1;Spieldatum/.*T(..:..).*/$1;Liga_Name_kurz;HeimTeam_Name_kurz;GastTeam_Name_kurz;Heim+%:+Gast" : "Spieldatum/^..(..).(..).(..).*/$3.$2.$1;Spieldatum/.*T(..:..).*/$1;HeimTeam_Name_kurz;GastTeam_Name_kurz;Heim+%:+Gast");
